...
async function executeScriptpoint(script: string, session: DebugSession, frameId: number): Promise<boolean> {
	return new Promise<boolean>(async (resolve, reject) => {
		...
		try {
			let f = Function('"use strict"; return async function(log, command, evaluate, variables, memory, vscode,Breakpoint,SourceBreakpoint,Position,window,debug,script,session,frameId){ ' + script + ' }')();
			await f(log, command, evaluate, variables, memory, vscode,Breakpoint,SourceBreakpoint,Position,window,debug,script,session,frameId);
		}
		catch (e) {
